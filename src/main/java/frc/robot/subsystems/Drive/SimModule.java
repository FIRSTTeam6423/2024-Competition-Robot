...
public class SimModule extends ModuleIO {
  ...
  // 2 states (position, velocity), 1 input (voltage), 2 output (position,
  // velocity)
  private LinearSystemSim<N2, N1, N2> driveMotor =
      new LinearSystemSim<N2, N1, N2>(
          LinearSystemId.createDCMotorSystem(
              DriveConstants.kV, // volts per meter per second
              DriveConstants.kA));

  // We aren't sure what the constants are for our pivot motors, but they work
  // fine with just P
  // controller
  // on real robot so as long as they work fine in the sim too that's okay. We are
  // mostly concerned
  // with
  // our drive motors.
  // private LinearSystemSim<N2, N1, N2> pivotMotor = new LinearSystemSim<N2, N1,
  // N2> (
  // LinearSystemId.createDCMotorSystem(
  // 2,
  // .2
  ...
  public SimModule(
    ...
    pivotPIDController = new PIDController(.25, Constants.MODULEPIVOT_I, Constants.MODULEPIVOT_D);
  ...
  public Rotation2d getPivotRotation() {
    // return
    // Rotation2d.fromDegrees(Math.toDegrees(pivotMotor.getAngularPositionRad()));
    // // -
  ...
  public SwerveModuleState getState() {
    return new SwerveModuleState(getDriveVelocity(), getPivotRotation());
  }
  ...
  public void setDesiredState(SwerveModuleState desiredState) {
    ...
    state = desiredState; // SwerveModuleState.optimize(desiredState,
    // Rotation2d.fromDegrees(curRotDeg));
