...
public class LEDSubsystem extends SubsystemBase {
  ...
  public Command setColor(Color color) {
    return this.runOnce(
        () -> {
          for (int i = 0; i < m_ledBuffer.getLength(); i++) {
            m_ledBuffer.setRGB(
                i, (int) (color.red * 255), (int) (color.green * 255), (int) (color.blue * 255));
          }
          m_led.setData(m_ledBuffer);
        });
  }
    ...
    return runOnce(
        ...
        .andThen(
            run(() -> {
                  ...
                  Color color =
                      new Color(
                          endColor.red * fadePercent,
                          endColor.green * fadePercent,
                          endColor.blue * fadePercent);
        ...
        .andThen(
            run(() -> {
                  ...
                  for (var i = 0; i < m_ledBuffer.getLength(); i++) {
                    // Calculate the hue - hue is easier for rainbows because
                    // the color
                    // shape is a circle so only one value needs to precess
                    final var hue =
                        (m_rainbowFirstPixelHue + (i * 180 * 7 / m_ledBuffer.getLength())) % 180;
                    // Set the value
                  ...
                  for (int i = 0; i < m_ledBuffer.getLength(); i++) {
                    if (i >= pingPongStart && i <= pingPongStart + lineLength) {
                      if (color == null) {
                        Color c =
                            Color.fromHSV(
                                (int) (((i - pingPongStart) / lineLength) * 180), 255, 255);
                        m_ledBuffer.setLED(i, c);
                      } else {
  ...
  public Command enabledIdle() {
    return pingPongRight(Color.kPurple, 4, 12)
        .repeatedly()
        .withInterruptBehavior(InterruptionBehavior.kCancelSelf);
  }
  ...
  public Command disabledIdle() {
    ...
    // return doublePingPong(Color.kGreen, Color.kGreen, 4,
    // 12).repeatedly().withTimeout(2).andThen(pingPongRight(Color.kPurple, 4,
    // 12)).andThen(pingPongRight(Color.kGreenYellow, 4,
    // 12)).andThen(pingPongLeft(Color.kAliceBlue,
    // 3, 12)).andThen(
    // fadeIn(Color.kGreen, .1).andThen(fadeOut(Color.kGreen,
    // .4)).repeatedly().withTimeout(3)
    ...
    ledEffectMap =
        Map.ofEntries(
            ...
            Map.entry(
                2, doublePingPong(Color.kGreen, Color.kGreen, 4, 12).repeatedly().withTimeout(2)),
            Map.entry(
                3,
                pingPongRight(null, 4, 20)
                    .andThen(pingPongRight(null, 2, 25))
                    .andThen(pingPongLeft(null, 4, 20))
                    .andThen(pingPongLeft(null, 2, 25))),
            Map.entry(
                4,
                fadeIn(Color.kGreen, .1)
                    .andThen(fadeOut(Color.kGreen, .4))
                    .repeatedly()
                    .withTimeout(2)));
    getRandomLEDEffect =
        new SelectCommand<>(ledEffectMap, () -> (int) (Math.random() * (ledEffectMap.size() + 1)));
    return getRandomLEDEffect
        .repeatedly()
